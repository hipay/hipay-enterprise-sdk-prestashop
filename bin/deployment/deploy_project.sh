...
sshpass -p $PASS_DEPLOY ssh root@docker-knock-auth.hipay.org -p $port  "export DOCKER_API_VERSION=1.23 && docker exec " \
    "jira-artifactory-pi.hipay-pos-platform" chmod u+x ./tmp/jfrog && ./tmp/jfrog rt u /deploy/project/artifactory/$CIRCLE_PROJECT_REPONAME/$BRANCH/*.zip "hipay/org.hipay/" \
