...
sshpass -p $PASS_DEPLOY ssh root@deploy.hipay-pos-platform.com mkdir /deploy/project/artifactory/$CI_PROJECT_NAME/$BRANCH

echo "Transfert Artifact project for project $CI_PROJECT_NAME and branch $GITHUB_BRANCH"
sshpass -p $PASS_DEPLOY scp ./package-ready-for-prestashop/*.zip root@deploy.hipay-pos-platform.com:/deploy/project/artifactory/$CI_PROJECT_NAME/$BRANCH
