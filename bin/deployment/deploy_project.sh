...
sshpass -p $PASS_DEPLOY ssh root@docker-knock-auth.hipay.org -p 32962 mkdir /deploy/project/artifactory/$CI_PROJECT_NAME/$BRANCH

echo "Transfert Artifact project for project $CI_PROJECT_NAME and branch $GITHUB_BRANCH"
sshpass -p $PASS_DEPLOY scp -p 32962 ./package-ready-for-prestashop/*.zip root@docker-knock-auth.hipay.org:/deploy/project/artifactory/$CI_PROJECT_NAME/$BRANCH
