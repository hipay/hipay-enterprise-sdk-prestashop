...
describe('Tests refund on MOTO', function () {
     ...
     */
    it('Refunds MOTO Transaction', function () {
        cy.logToAdmin();
        ...
        cy.fixture('customerFR').then((customer) => {
            ...
            cy.wait(5000).get(".fancybox-inner").find("iframe").then(($iframe) => {
                ...
                cy.wait(5000).get(".fancybox-inner").find("iframe").then(($iframe) => {
                    ...
                    cy.get('input[name="cart_id"]').then((cartIdElement) => {
                        let cartId = cartIdElement.val();
                        cy.window().then((win) => {
                            let orderId = new URL(win.location.href).searchParams.get('id_order');
                            ...
                            cy.openTransactionOnHipayBO(cartId + "(");
                            ...
                            cy.logToAdmin();
                            cy.goToDetailOrder(orderId);
                            cy.get('button[name="hipay_refund_basket_submit"]').click();
                            cy.get('.alert-success').contains("The refund has been validated").then(($el) => {
                                expect($el.text()).to.match(/The refund has been validated/);
                            });

                            cy.connectOnHipayBOFromAnotherDomain();
                            cy.openTransactionOnHipayBO(cartId + "(");
                            cy.get('#data-status-message').then(($el) => {
                                expect($el.text()).to.match(/Refund Requested/);
                            });
                        });
                    });
                });
