...
if [ "$1" = "0" ];then
    ...
    casperjs test ${header}000*/*/*/*.js ${header}000*/000[0-2]*.js ${header}0[0-1][0-9]*/[0-1]*/[0-9][0-3][0-9][0-9]-*.js --url=$BASE_URL/ --url-mailcatcher=$URL_MAILCATCHER --login-backend=$LOGIN_BACKEND --pass-backend=$PASS_BACKEND --login-paypal=$LOGIN_PAYPAL --pass-paypal=$PASS_PAYPAL --xunit=${header}result.xml --ignore-ssl-errors=true --ssl-protocol=any
fi

if [ "$1" = "1" ];then
    echo "Execute part $1 of casperjs test"
    casperjs test ${header}000*/*/*/*.js ${header}000*/000[0-2]*.js ${header}0[0-1][0-9]*/[0-1]*/[0-9][4-9][0-9][0-9]-*.js --url=$BASE_URL/ --url-mailcatcher=$URL_MAILCATCHER --login-backend=$LOGIN_BACKEND --pass-backend=$PASS_BACKEND --login-paypal=$LOGIN_PAYPAL --pass-paypal=$PASS_PAYPAL --xunit=${header}result.xml --ignore-ssl-errors=true --ssl-protocol=any
fi
