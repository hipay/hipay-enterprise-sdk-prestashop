...
casper.test.begin('Test admin configuration screens', function(test) {
    ...
    .then(function() {
        ...
        this.waitForSelector('form#account_form', function success() {
            ...
            test.assertExists('form#account_form input[name="api_username_production"]');
            test.assertExists('form#account_form input[name="api_password_production"]');
            test.assertExists('form#account_form input[name="api_secret_passphrase_production"]');
            test.assertExists('form#account_form input[name="api_tokenjs_username_production"]');
            test.assertExists('form#account_form input[name="api_tokenjs_password_publickey_production"]');
            test.assertExists('form#account_form input[name="api_moto_username_production"]');
            test.assertExists('form#account_form div.alert');

            test.assertExists('form#account_form div.alert');
            test.assertExists('form#account_form input[name="api_username_sandbox"]');
            test.assertExists('form#account_form input[name="api_password_sandbox"]');
            test.assertExists('form#account_form input[name="api_secret_passphrase_sandbox"]');
            test.assertExists('form#account_form input[name="api_tokenjs_username_sandbox"]');
            test.assertExists('form#account_form input[name="api_tokenjs_password_publickey_sandbox"]');
            test.assertExists('form#account_form input[name="api_moto_username_sandbox"]');
    ...
    .then(function() {
        ...
        this.waitForSelector('ul.hipay-enterprise li a[href="#payment_form"]', function success() {
            ...
            test.assertExists('form#credit_card_form select[name="operating_mode"]');
            test.assertExists('form#credit_card_form select#capture_mode.col-lg-2');
            test.assertExists('form#credit_card_form input#card_token_switchmode_on');
            test.assertExists('form#credit_card_form input#activate_basket_switchmode_on');
            test.assertExists('form#credit_card_form input#regenerate_cart_on_decline_switchmode_on');
            test.assertExists('form#credit_card_form input#log_infos_switchmode_on');
            test.assertExists('form#credit_card_form select[name="activate_3d_secure"]');
            ...
            test.assertExists('form#credit_card_form input[name="visa_minAmount[EUR]"]');
            test.assertExists('form#credit_card_form input[name="visa_maxAmount[EUR]"]');
            test.assertExists('form#credit_card_form select[name="visa_currencies[]"]');
            test.assertExists('form#credit_card_form select[name="visa_countries[]_helper1"]');
            test.assertExists('form#credit_card_form button[name="creditCardSubmit"]');
            ...
            test.assertExists('div#panel-local-payment #oney.active input[name="3xcb-no-fees_currencies[]"]');
            test.assertExists('div#panel-local-payment #oney.active input[name="3xcb-no-fees_countries[]"]');
        })
    })
    .then(function() {
        /* Panel Fraud method */
        this.echo("Test panel : 'Fraud'", "INFO");
        this.waitForSelector('ul.hipay-enterprise li a[href="#fraud"]', function success() {
            this.click('ul.hipay-enterprise li a[href="#fraud"]');
            test.assertExists('form#configuration_form');
            test.assertExists('form#configuration_form input[name="send_payment_fraud_email_copy_to"]');
            test.assertExists('form#configuration_form select[name="send_payment_fraud_email_copy_method"]');
            test.assertExists('form#configuration_form button[name="fraudSubmit"]');
        })
     })
    .then(function() {
        /* Panel FAQ method */
        this.echo("Test panel : 'FAQ'", "INFO");
        this.waitForSelector('ul.hipay-enterprise li a[href="#faq"]', function success() {
    ...
    .then(function() {
        ...
        this.waitForSelector('ul.hipay-enterprise li a[href="#logs"]', function success() {
