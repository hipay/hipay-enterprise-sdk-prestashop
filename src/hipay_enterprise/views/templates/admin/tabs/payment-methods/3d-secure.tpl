    ...
    <form method="post" class="defaultForm form-horizontal" action="{$smarty.server.REQUEST_URI|escape:'htmlall':'UTF-8'}" id="credit_card_form">
        <div class="panel" id="fieldset_0">
                ...
                <div class="form-group">
                    ...
                    <div class="col-lg-2">
                        <select name="3d_secure_rules[total_price][operator]"  id="3d_secure_rules">
                            <option value=">" {if $config_hipay.payment.global.3d_secure_rules[0].operator|@html_entity_decode == ">"}selected="selected" {/if} >{l s='Greater than' mod='hipay_enterprise'}</option>
                            <option value=">=" {if $config_hipay.payment.global.3d_secure_rules[0].operator|@html_entity_decode == ">="}selected="selected" {/if} >{l s='Greater than or equals to' mod='hipay_enterprise'}</option>
                            <option value="<" {if $config_hipay.payment.global.3d_secure_rules[0].operator|@html_entity_decode == "<"}selected="selected" {/if} >{l s='Lower than' mod='hipay_enterprise'}</option>
                            <option value="<=" {if $config_hipay.payment.global.3d_secure_rules[0].operator|@html_entity_decode == "<="}selected="selected" {/if} >{l s='Lower than or equals to' mod='hipay_enterprise'}</option>
                            <option value="==" {if $config_hipay.payment.global.3d_secure_rules[0].operator|@html_entity_decode == "=="}selected="selected" {/if} >{l s='Equals to' mod='hipay_enterprise'}</option>
                            <option value="!=" {if $config_hipay.payment.global.3d_secure_rules[0].operator|@html_entity_decode == "!="}selected="selected" {/if} >{l s='Not equals to' mod='hipay_enterprise'}</option>
                        </select>
                    </div>
                    <div class="col-lg-3">
            ...
            <div class="panel-footer">
                <button type="submit" class="btn btn-default btn btn-default pull-right" name="submit3DSecure"><i class="process-icon-save"></i> {l s='Save' mod='hipay_enterprise'}</button>
            </div>
        </div><!-- /.form-wrapper -->
    </form>
</div>
