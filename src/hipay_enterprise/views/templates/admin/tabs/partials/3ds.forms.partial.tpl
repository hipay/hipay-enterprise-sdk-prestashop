<div class="form-group">
        ...
        <div class="col-lg-9">
                <select name="activate_3d_secure" class="col-lg-3" id="activate_3d_secure">
                        <option value="0" {if $HiPay_config_hipay.payment.global.activate_3d_secure == "0"}selected="selected"
                                {/if}>{l s='Disabled' mod='hipay_enterprise'}</option>
                        <option value="1" {if $HiPay_config_hipay.payment.global.activate_3d_secure == "1"}selected="selected"
                                {/if}>{l s='Try to enable for all transactions' mod='hipay_enterprise'}</option>
                        <option value="2" {if $HiPay_config_hipay.payment.global.activate_3d_secure == "2"}selected="selected"
                                {/if}>{l s='Try to enable for configured 3ds rules' mod='hipay_enterprise'}</option>
                        <option value="3" {if $HiPay_config_hipay.payment.global.activate_3d_secure == "3"}selected="selected"
                                {/if}>{l s='Force for configured 3ds rules' mod='hipay_enterprise'}</option>
                        <option value="4" {if $HiPay_config_hipay.payment.global.activate_3d_secure == "4"}selected="selected"
        ...
        <div class="form-group">
                ...
                <div class="col-lg-2">
                        <select name="3d_secure_rules[total_price][operator]" id="3d_secure_rules">
                                <option value=">"
                                        {if $HiPay_config_hipay.payment.global.3d_secure_rules[0].operator|htmlEntityDecode == ">"}selected="selected"
                ...
                <div class="col-lg-3">
                        <input type="text" name="3d_secure_rules[total_price][value]"
                                value="{$HiPay_config_hipay.payment.global.3d_secure_rules[0].value}">
                </div>
        </div>
</div>