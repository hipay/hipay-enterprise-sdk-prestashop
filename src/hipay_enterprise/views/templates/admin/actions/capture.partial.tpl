...
<fieldset>
    ...
    <div style="display: flex;">
        <p><b>{l s='Amount already captured' mod='hipay_enterprise' } :</b> <span
                class="badge {if $HiPay_capturedAmount > 0}badge-warning{else}badge-success{/if}">{displayPrice price=$HiPay_capturedAmount currency=$HiPay_id_currency}</span></p>
    </div>
    <div>
    ...
    <form action="{$HiPay_captureLink}" method="post" id="hipay_capture_form" class="">
        ...
        <div class="form-group">
                ...
                <button id="basketSubmitCaptureButton" style="display:none;" type="submit" name="hipay_capture_basket_submit"
                    class="btn btn-primary pull-right">
                    {l s='Capture'  mod='hipay_enterprise'}
                </button>
        </div>
...
<script>
    $(document).ready(function () {
        ...
        if ($("#hipay_capture_type option:selected").val() == "partialWithoutBasket") {
            if (parseFloat($("#hipay_capture_amount").val()) == 0 || $("#hipay_capture_amount").val() >
                refundableAmount) {
