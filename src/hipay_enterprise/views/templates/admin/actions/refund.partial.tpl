...
<fieldset>
    ...
    <p class="help-block">
        <sup>*</sup>
        {l s='Amount will be updated once the refund will be confirmed by HiPay Enterprise' mod='hipay_enterprise'}
    </p>
                ...
                <table class="table table-item-hipay refund-table">
                    ...
                    {foreach $HiPay_products as $item}
                        ...
                        <tr>
                            ...
                            <td>
                                <input type="hidden" {if $remainQty == 0} disabled {/if} name="hipayrefund[{$itemId}]"
                                    value="{$itemId}" />
                                {$item["product_name"]}
                            </td>
                            <td>
                                ...
                                {if $remainQty > 0}
                                    <div class="input-group refund-quantity-selector">
                                        <input data-unit-price="{$item.unit_price_tax_incl}" data-id="{$itemId}"
                                            class="good-selector-refund" name="hipayrefund[{$itemId}]" type="number" min="0"
...
<script>
    ...
    function checkRefundAmount() {
        ...
        if ($("#hipay_refund_type option:selected").val() == "partial") {
            ...
            if (!$("#refund-discount").is(":checked") && (refundableAmount - parseFloat($("#total-refund-input")
                    .val()) <= parseFloat($("#refund-discount").data("amount")))) {
    ...
    function addEvents() {
        $(".good-selector-refund").on('keyup change click', function() {
            setTimeout(updateRefundPrice, 500);
        });

        $("#refund-fee").click(updateRefundPrice);
        $("#refund-discount").click(updateRefundPrice);
        $("#refund-wrapping").click(updateRefundPrice);
