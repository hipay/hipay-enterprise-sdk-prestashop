...
abstract class RequestFormatterAbstract extends CommonRequestFormatterAbstract
{
    ...
    protected function mapRequest(&$order)
    {
        ...
        $order->accept_url           = $this->context->link->getModuleLink($this->module->name,
            'validation', array(), true);
        $order->decline_url          = $this->context->link->getModuleLink($this->module->name,
            'decline', array(), true);
        $order->pending_url          = $this->context->link->getModuleLink($this->module->name,
            'pending', array(), true);
        $order->exception_url        = $this->context->link->getModuleLink($this->module->name,
            'exception', array(), true);
        $order->cancel_url           = $this->context->link->getModuleLink($this->module->name,
            'cancel', array(), true);
    ...
    protected function generateDescription($order)
    {
        ...
        if (Tools::strlen($description) > $max_length) {
            ...
            $description = Tools::substr($description, 0,
                    strrpos($description, ' ', $offset)).'...';
        }
