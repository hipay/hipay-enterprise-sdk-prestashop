...
class CartMaintenanceFormatter implements ApiFormatterInterface
{
    ...
    private function getGoodItem($product, $qty)
    {
        ...
        $item->__constructItem($european_article_numbering, $product_reference,
            $type, $name, $qty, $unit_price, $tax_rate, $discount,
            $total_amount, $discount_description, $product_description,
            $delivery_method, $delivery_company, $delivery_delay,
            $delivery_number, $product_category, $shop_id);
    ...
    private function getDiscountItem($totalQty)
    {
        ...
        $item = HiPay\Fullservice\Gateway\Model\Cart\Item::buildItemTypeDiscount($product_reference,
                $name, $unit_price, $tax_rate, $discount, $discount_description,
                $total_amount);
        // forced category
    ...
    private function getFeesItem()
    {
        ...
        $item              = HiPay\Fullservice\Gateway\Model\Cart\Item::buildItemTypeFees($product_reference,
                $name, $unit_price, $tax_rate, $discount, $total_amount);
        // forced category
        $item->setProductCategory(1);
