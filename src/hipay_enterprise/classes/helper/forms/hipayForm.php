...
class HipayForm extends HipayFormInput
{
    ...
    public function __construct($module_instance)
    {
        ...
        $this->helper->currentIndex = $this->context->link->getAdminLink('AdminModules',
            false);
    ...
    public function getGlobalPaymentMethodsForm()
    {
        ...
        $form['form']['input'][] = $this->generateInputSelect(
            "operating_mode", $this->module->l('Operating mode', 'HipayForm'),
            array(
            "options" => array(
                "query" => array(
                    array(
        ...
        $form['form']['input'][] = $this->generateInputSelect(
            ...
            array(
            'hint' => $this->module->l('Basic hosted page or iFrame template.',
                'HipayForm'),
            "options" => array(
                "query" => array(
                    array(
        ...
        $form['form']['input'][] = $this->generateInputSelect(
            ...
            array(
            'hint' => $this->module->l('Display card selector on iFrame or hosted page.',
                'HipayForm'),
            "options" => array(
                "query" => array(
                    array(
                        "display_card_selector_id" => 1,
                        "name" => $this->module->l('Show card selector',
                            'HipayForm')
                    ),
                    array(
                        "display_card_selector_id" => 0,
                        "name" => $this->module->l('Do not show card selector',
                            'HipayForm')
                    ),
                ),
        ...
        $form['form']['input'][] = $this->generateInputText(
            "css_url", $this->module->l('CSS url', 'HipayForm'),
            array(
            'hint' => $this->module->l('URL to your CSS (style sheet) to customize your hosted page or iFrame (Important: the HTTPS protocol is required)',
                'HipayForm'),
            )
        );
...
//        );
        ...
        $form['form']['input'][] = $this->generateInputSelect(
            ...
            "options" => array(
                "query" => array(
                    array(
                        "3_d_mode_id" => HipayConfig::THREE_D_S_DISABLED,
                        "name" => $this->module->l('Disabled', 'HipayForm')
                    ),
                    array(
                        "3_d_mode_id" => HipayConfig::THREE_D_S_TRY_ENABLE_ALL,
                        "name" => $this->module->l('Try to enable for all transactions',
                            'HipayForm')
                    ),
                    array(
                        "3_d_mode_id" => HipayConfig::THREE_D_S_TRY_ENABLE_RULES,
                        "name" => $this->module->l('Try to enable for configured 3ds rules',
                            'HipayForm')
                    ),
                    array(
                        "3_d_mode_id" => HipayConfig::THREE_D_S_FORCE_ENABLE_RULES,
                        "name" => $this->module->l('Force for configured 3ds rules',
                            'HipayForm')
                    ),
                    array(
                        "3_d_mode_id" => HipayConfig::THREE_D_S_FORCE_ENABLE_ALL,
                        "name" => $this->module->l('Force for all transactions',
                            'HipayForm')
                    ),
                ),
                "id" => "3_d_mode_id",
        ...
        $form['form']['input'][] = $this->generateSwitchButton(
            ...
            array(
            'hint' => $this->module->l('Allow users to save their card and use saved cards.',
                'HipayForm'),
            )
        );
        ...
        $form['form']['input'][] = $this->generateSwitchButton("electronic_signature",
            $this->module->l('Activate electronic signature', 'HipayForm'));

        $form['form']['input'][] = $this->generateSwitchButton("activate_basket",
            $this->module->l('Activate basket', 'HipayForm'),
            array(
            'hint' => $this->module->l('Send cart informations on Hipay API call.',
                'HipayForm'),
        ...
        $form['form']['input'][] = $this->generateSwitchButton(
            ...
            $this->module->l('Keep cart when payment fail', 'HipayForm')
        );
    ...
    public function getFraudForm()
    {
        ...
        $form['form']['input'][] = $this->generateInputText(
            ...
            $this->module->l('Payment Fraud Email Sender', 'HipayForm')
        );

        $form['form']['input'][] = $this->generateInputText(
            "send_payment_fraud_email_copy_to",
            $this->module->l('Send Payment Fraud Email Copy To', 'HipayForm')
        );

        $form['form']['input'][] = $this->generateInputSelect(
            "send_payment_fraud_email_copy_method",
            $this->module->l('Send Payment Fraud Email Copy Method', 'HipayForm'),
            array(
            "options" => array(
                "query" => array(
                    array(
        ...
        $form['form']['input'][] = $this->generateInputText(
            ...
            $this->module->l('Payment Fraud Accept Email Sender', 'HipayForm')
        );

        $form['form']['input'][] = $this->generateInputText(
            "send_payment_accept_email_copy_to",
            $this->module->l('Send Payment Accept Email Copy To', 'HipayForm')
        );

        $form['form']['input'][] = $this->generateInputSelect(
            "send_payment_accept_email_copy_method",
            $this->module->l('Send Payment Accept Email Copy Method',
                'HipayForm'),
            array(
            "options" => array(
                "query" => array(
        ...
        $form['form']['input'][] = $this->generateInputText(
            ...
            $this->module->l('Payment Fraud Deny Email Sender', 'HipayForm')
        );

        $form['form']['input'][] = $this->generateInputText(
            "send_payment_deny_email_copy_to",
            $this->module->l('Send Payment Deny Email Copy To', 'HipayForm')
        );

        $form['form']['input'][] = $this->generateInputSelect(
            "send_payment_deny_email_copy_method",
            $this->module->l('Send Payment Deny Email Copy Method', 'HipayForm'),
            array(
            "options" => array(
                "query" => array(
                    array(
