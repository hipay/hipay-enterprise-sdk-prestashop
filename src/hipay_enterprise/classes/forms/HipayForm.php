...
class HipayForm extends HipayFormInput
{
    ...
    public function getFraudForm()
    {
        ...
        $form['form']['input'][] = $this->generateInputText(
            "send_payment_fraud_email_copy_to", $this->module->l(
                'Copy To', 'HipayForm'
            ),
                array(
            'desc' => $this->module->l(
        ...
        $form['form']['buttons'][] = $this->generateSubmitButton(
            $this->module->l(
                'Save configuration changes', 'HipayForm'
            ), array(
            'name' => 'fraudSubmit',
    ...
    public function getFraudFormValues()
    {
        // init field
        $values = array(
            "input_split" => $this->generateHtmlNoticeAdmin('When a transaction is likely to be a fraud then an email is sent to the contact email from your shop as well as to an additional sender. Here you can configure the additional recipient email'),
